...
namespace Aspenlaub.Net.GitHub.CSharp.Backbend.Test {
    ...
    public class BackbendFoldersAnalyserTest {
        ...
        public void CanAnalyseBackbendFolders() {
            ...
            secretRepositoryMock.Setup(s => s.Get(It.IsAny<ISecret<BackbendFolders>>())).Returns(backbendFolders);
            var secret = new ArchiveFolderFinderSecret();
            secretRepositoryMock.Setup(s => s.Get(It.IsAny<ISecret<PowershellFunction<string, string>>>())).Returns(secret.DefaultValue);
            ...
            var result = sut.Analyse().ToList();
            Assert.AreEqual(1, result.Count);

            File.WriteAllText(archiveFileName, textFileName);

            result = sut.Analyse().ToList();
